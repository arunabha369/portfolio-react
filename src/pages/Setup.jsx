...
export default function SetupPage() {
  return <Container className="py-8 md:py-16">
    <Helmet>
      <title>Setup | Arunabha Banerjee</title>
    </Helmet>
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="space-y-8 md:space-y-12">
        {steps.map(step => <div key={step.id} className="space-y-4 md:space-y-6">
          {/* Step Header */}
          ...
          <div className="ml-4 space-y-3 sm:ml-8 md:ml-16 md:space-y-4">
            {step.content.map((item, index) => <div key={index} className="flex items-start gap-3">
              {item.type === 'download' && <Link href={item.href || '#'} download className="bg-muted/50 hover:bg-muted/70 flex w-full flex-col gap-3 rounded-lg border border-black/10 p-3 transition-colors sm:flex-row sm:items-center md:p-4 dark:border-white/10">
              ...
              {item.type === 'instruction' && <div className="flex items-center gap-3">
                <p className="text-secondary text-sm">{item.text}</p>
              </div>}

              {item.type === 'shortcut' && <div className="bg-accent/50 flex w-full items-center gap-3 rounded-lg border border-black/10 p-3 dark:border-white/10">
                <kbd className="bg-muted rounded border border-black/20 px-2 py-1 font-mono text-xs break-all dark:border-white/20">
                  {item.text}
                </kbd>
              </div>}

              {item.type === 'prompt' && <div className="bg-muted/50 flex w-full items-center gap-3 rounded-lg border border-black/10 p-3 dark:border-white/10">
                  ...
                  <code className="language-json text-secondary block font-mono leading-relaxed whitespace-pre">
                    {settingsJson.split('\n').map((line, index) => {
                      ...
                      if (trimmedLine.startsWith('//')) {
                        return <div key={index} className="text-muted-foreground/60 italic">
                          {line}
                        </div>;
