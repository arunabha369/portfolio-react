...
function generateSystemPrompt() {
  ...
  const socialLinksText = socialLinks.map(link => `${link.name}: ${link.href}`).join('\n- ');
  const experienceText = experiences.map(exp => `${exp.position} at ${exp.company} (${exp.startDate} - ${exp.endDate})`).join('\n- ');
  const projectsText = projects.map(project => `${project.title}: ${project.description}${project.live ? ` - ${project.live}` : ''}`).join('\n- ');
...
export const chatSuggestions = ['What technologies do you work with?', 'Tell me about your recent projects', 'How can I contact you for work?'];