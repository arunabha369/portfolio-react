...
const ChatBubble = () => {
  ...
  return <ExpandableChat className="mt-4 ml-4 max-h-[95vh] max-w-[calc(100vw-2rem)] hover:cursor-pointer sm:max-w-[calc(100vw-4rem)] md:max-w-xl" position="bottom-right" size="lg" icon={<ChatBubbleIcon className="h-6 w-6" />}>
    <ExpandableChatHeader>
      <div className="flex items-center space-x-3">
        <Avatar className="border-primary h-8 w-8 border-2 bg-black">
        ...
        <div className="space-y-4">
                ...
                <div className="flex items-center gap-2">
                  <div className="prose prose-sm dark:prose-invert max-w-none flex-1">
                    {message.text ? <ReactMarkdown components={{
                      ...
                      {message.text}
                    </ReactMarkdown> : message.isStreaming && <span className="text-muted-foreground">
                      Thinking...
                    </span>}
                  </div>
                </div>
                <p className={cn('mt-1 text-xs', message.sender === 'user' ? 'text-secondary' : 'text-muted-foreground')}>
                  {message.timestamp}
                </p>
              </div>
            </div>
          ...
          {messages.length === 1 && !isLoading && <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2 px-3">
              {chatSuggestions.map((suggestion, index) => <Button key={index} variant="outline" size="sm" onClick={() => handleSuggestionClick(suggestion)} className="bg-background hover:bg-muted border-muted-foreground/20 h-8 px-3 text-xs">
                {suggestion}
              </Button>)}
            </div>
          </div>}
